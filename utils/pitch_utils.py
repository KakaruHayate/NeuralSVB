...
def norm_f0(f0, uv, hparams):
    ...
    if hparams.get('pitch_norm') == 'standard':
        if hparams.get('f0_mean') is not None:
            f0 = (f0 - hparams['f0_mean']) / hparams['f0_std']
        # else: skip normalization (mean/std not available during inference)
    if hparams.get('pitch_norm') == 'log':
        f0 = torch.log2(f0 + 1e-8) if is_torch else np.log2(f0 + 1e-8)
    if uv is not None and hparams.get('use_uv', False):
        f0[uv > 0] = 0
    return f0
...
def denorm_f0(f0, uv, hparams, pitch_padding=None, min=None, max=None):
    ...
    if hparams.get('pitch_norm') == 'standard':
        ...
        # else: skip denormalization during inference
    if hparams.get('pitch_norm') == 'log':
        f0 = 2 ** f0
    if min is None:
        min = 0
